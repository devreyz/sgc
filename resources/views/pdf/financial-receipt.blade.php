...
<body>
    ...
    <div class="strip"><div class="strip-cell"><span class="label">Pagador</span><span class="value">{{ $receipt->payer_name }}</span>@if ($receipt->payer_document)<div class="detail-text">Documento: {{ $receipt->payer_document }}</div>@endif</div><div class="strip-cell"><span class="label">Meio de pagamento</span><span class="value">{{ $receipt->payment_method->getLabel() }}</span>@if ($receipt->payment_reference)<div class="detail-text">Ref.: {{ $receipt->payment_reference }}</div>@endif</div></div>

    <div class="declaration">Recebemos de <strong>{{ $receipt->payer_name }}</strong> a importancia de <strong>R$ {{ number_format((float) $receipt->total_amount, 2, ',', '.') }}</strong> ({{ $amountInWords }}). @if ($receipt->purpose)O recebimento refere-se a: <strong>{{ $receipt->purpose }}</strong>.@endif</div>

    @if ($hasItems)
        <div class="section-title">Detalhamento</div>
        <table class="tbl"><thead><tr><th style="width:5%">#</th><th>Descricao / referencia</th><th class="r">Qtd.</th><th>Un.</th><th class="r">Vlr. unit.</th><th class="r">Total</th></tr></thead><tbody>@foreach ($receipt->items as $item)<tr><td>{{ $loop->iteration }}</td><td>{{ $item->description }}@if ($item->reference)<br><span style="color:#687078;font-size:8px">Ref.: {{ $item->reference }}</span>@endif</td><td class="r">{{ rtrim(rtrim(number_format((float) $item->quantity, 4, ',', '.'), '0'), ',') }}</td><td>{{ $item->unit }}</td><td class="r">R$ {{ number_format((float) $item->unit_price, 4, ',', '.') }}</td><td class="r">R$ {{ number_format((float) $item->total_amount, 2, ',', '.') }}</td></tr>@endforeach</tbody><tfoot><tr><td colspan="5" class="r">TOTAL</td><td class="r">R$ {{ number_format((float) $receipt->total_amount, 2, ',', '.') }}</td></tr></tfoot></table>
    @endif

    <div class="details"><div class="details-col"><div class="detail-row"><span class="label">Conta de entrada</span><span class="detail-text">{{ $receipt->bankAccount?->name ?? 'Conta nao identificada' }}</span></div><div class="detail-row"><span class="label">Classificacao</span><span class="detail-text">{{ $receipt->chartAccount ? $receipt->chartAccount->code.' - '.$receipt->chartAccount->name : 'Nao informada' }}</span></div></div><div class="details-col"><div class="detail-row"><span class="label">Observacoes</span><span class="detail-text">{{ $receipt->notes ?: 'Sem observacoes.' }}</span></div>@if ($documentStatus === 'cancelled')<div class="detail-row"><span class="label">Motivo do cancelamento</span><span class="detail-text">{{ $receipt->cancellation_reason }}</span></div>@endif</div></div>

    <div class="signatures"><div class="signature"><div class="signature-line">{{ $receiverName ?: 'Membro nao identificado' }}</div><div class="signature-role">Recebedor</div></div><div class="signature"><div class="signature-line">{{ $receipt->payer_name }}</div><div class="signature-role">Pagador</div></div></div>
