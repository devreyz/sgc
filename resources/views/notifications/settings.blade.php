            ...
            <section class="event-group">
                ...
                @foreach($events as $key => $event)
                    ...
                    <article class="bento-card event-row">
                        ...
                        <label class="event-switch" title="{{ $event['pushAllowed'] ? 'Enviar ao aplicativo Android' : 'Notificacao Android bloqueada para eventos editaveis' }}"><input type="hidden" name="events[{{ $key }}][push_enabled]" value="0"><input type="checkbox" name="events[{{ $key }}][push_enabled]" value="1" @checked($event['pushAllowed'] && ($saved?->push_enabled ?? $event['pushDefault'])) @disabled(!$event['pushAllowed'])> Android</label>
