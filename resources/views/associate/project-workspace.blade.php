...
(() => {
    ...
    function renderSummary(data){
        ...
        const breakdownHtml=breakdown.length?`<div class="fee-breakdown">${breakdown.map(f=>`<span class="fee-chip">${esc(f.label||f.name||'Taxa')} <strong>${f.nature==='accrual'?'+':'-'}${money(Math.abs(Number(f.amount||0)))}</strong></span>`).join('')}</div>`:'';
    ...
    function renderDeliveries(data){
        ...
        const rows=records.map(item=>{
            ...
            const rowTone=item.status==='rejected'||item.status==='cancelled'?'row-danger':item.status==='pending'||remaining>0?'row-warning':'';
            return `<div class="table-row ${rowTone}">
                <div class="table-cell" data-label="Data"><strong>${esc(item.date||'-')}</strong></div>
                <div class="table-cell product-wide" data-label="Produto"><div class="product-cell"><span class="product-icon"><i class="ph-fill ph-package"></i></span><div class="product-copy"><strong>${esc(item.product)}</strong><span>${esc(item.quality||'Qualidade não informada')}</span></div></div></div>
                <div class="table-cell" data-label="Status">${badge(item.status,item.status_label)}</div>
                <div class="table-cell value" data-label="Entregue">${qty(item.quantity)} ${esc(item.unit)}</div>
                <div class="table-cell value" data-label="Destinado">${qty(item.distributed)} ${esc(item.unit)}</div>
                <div class="table-cell value net-col" data-label="Líquido">${Number(item.distribution_count||0)>0?money(item.net):'—'}</div>
                ${(remaining>0||note||reject)?`<div class="row-extra">${remaining>0?`<span class="status-badge pending">${qty(remaining)} ${esc(item.unit)} sem destino</span>`:''}${note?`<button type="button" class="note-btn" data-aw-note="${esc(note)}" data-aw-note-title="Observações da entrega" data-aw-note-meta="${esc(`${item.product||'Produto'} · ${item.date||''}`)}"><i class="ph ph-note"></i>Observações</button>`:''}${reject?`<button type="button" class="note-btn" data-aw-note="${esc(reject)}" data-aw-note-title="Avaliação de qualidade" data-aw-note-meta="${esc(`${item.product||'Produto'} · ${item.date||''}`)}"><i class="ph ph-warning-circle"></i>Avaliação</button>`:''}</div>`:''}
            </div>`;
        }).join('');
        root.innerHTML=`<section class="section-shell">${sectionHead(`${records.length} ${records.length===1?'registro nesta página':'registros nesta página'}`,'deliveries')}<div class="section-body">${guide('deliveries')}${tools('deliveries')}${rows?`<div class="data-table deliveries-table"><div class="table-head"><span>Data</span><span>Produto</span><span>Status</span><span>Entregue</span><span>Destinado</span><span class="net-col">Líquido</span></div>${rows}</div>`:empty('Nenhuma entrega encontrada','Ajuste os filtros ou aguarde novas entregas.','ph-package')}${pager(data)}</div></section>`;
    }
    ...
    function renderDistributions(data){
        ...
        const rows=records.map(item=>`<div class="table-row">
            <div class="table-cell" data-label="Data"><strong>${esc(item.date||'-')}</strong></div>
            <div class="table-cell product-wide" data-label="Produto"><div class="product-cell"><span class="product-icon" style="--row-tone:var(--blue);--row-soft:var(--blue-soft)"><i class="ph-fill ph-map-pin"></i></span><div class="product-copy"><strong>${esc(item.product)}</strong><span>${item.receipt?'Em comprovante':'Pendente de comprovante'}</span></div></div></div>
            <div class="table-cell" data-label="Destino"><strong>${esc(item.customer||'Destino não informado')}</strong></div>
            <div class="table-cell value" data-label="Quantidade">${qty(item.quantity)} ${esc(item.unit)}</div>
            <div class="table-cell value" data-label="Preço">${money(item.unit_price)}</div>
            <div class="table-cell value" data-label="Bruto">${money(item.gross)}</div>
            <div class="table-cell receipt-col" data-label="Comprovante">${item.receipt?`<span class="status-badge approved">${esc(item.receipt)}</span>`:'<span class="status-badge pending">Pendente</span>'}</div>
            ${String(item.notes||'').trim()?`<div class="row-extra"><button type="button" class="note-btn" data-aw-note="${esc(item.notes)}" data-aw-note-title="Observações da distribuição" data-aw-note-meta="${esc(`${item.product||'Produto'} · ${item.customer||'Cliente'} · ${item.date||''}`)}"><i class="ph ph-note"></i>Observações</button></div>`:''}
        </div>`).join('');
        root.innerHTML=`<section class="section-shell">${sectionHead(`${records.length} ${records.length===1?'destino nesta página':'destinos nesta página'}`,'distributions')}<div class="section-body">${guide('distributions')}${tools('distributions')}${rows?`<div class="data-table distributions-table"><div class="table-head"><span>Data</span><span>Produto</span><span>Destino</span><span>Quantidade</span><span>Preço</span><span>Bruto</span><span class="receipt-col">Comprovante</span></div>${rows}</div>`:empty('Nenhuma entrega por destino encontrada','Ainda não há destinos para os filtros selecionados.','ph-map-pin')}${pager(data)}</div></section>`;
    }
